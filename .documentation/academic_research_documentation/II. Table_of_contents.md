
ACKNOWLEDGEMENT

The completion of the Gym Membership Management System (GyMMS) was made possible through the collective support, expertise, and encouragement of many individuals and organizations. I am especially grateful to Mr. Adrian Dave Ignacio, my project adviser, for his technical guidance, constructive feedback, and mentorship, which were instrumental in shaping the system’s architecture and ensuring its robustness. His insights into best practices and careful review of my work greatly enhanced the quality of this project.


I am deeply appreciative of the client gym operator, whose willingness to share operational insights, participate in requirement validation, and provide honest feedback during testing ensured that my solution addressed real-world needs. Their collaboration exemplified the value of academic-industry partnerships.

Special recognition is due to my family for their patience, sacrifices, and unwavering encouragement throughout the demanding development process. Their support enabled me to persevere through challenges and long hours. I also thank my peers and colleagues for their collaborative spirit, technical assistance, and shared commitment to learning, which enriched my experience and contributed to problem-solving.

Finally, I acknowledge with gratitude the providence and clarity that guided my efforts. Any shortcomings are my own, but the achievements of this project are shared with all who contributed to its realization.



TABLE OF CONTENTS

ACKNOWLEDGEMENT
TABLE OF CONTENTS
LIST OF FIGURES
LIST OF TABLES
ABSTRACT
1 EXECUTIVE SUMMARY
2 INTRODUCTION
2.1 Project Background
2.2 Client Overview
2.3 Project Objectives
2.4 Significance of the Project
2.5 Scope and Delimitations
3 SYSTEM ANALYSIS
2.1 Client Requirements
2.2 Functional Requirements
2.3 Non-Functional Requirements
2.4 Use Cases / User Stories
2.5 System Architecture
2.5.2 System Specifications
Table xx. Some Captions
4 DESIGN AND IMPLEMENTATION
3.1 Django Project Structure
Figure 3.1 Project Structure
Brief discussion of the 3.1
3.2 Forms and Validation
Figure 3.2 XXXXXXXX
3.3 Backend API
3.4 Web Application
3.4.1 Functional View
3.4.2 Database Design
3.4.3 Dataflow Diagram Level 0 and 1
3.4.4 User Interface and Features
5 TESTING AND DEBUGGING
4.1 Backend API Testing
4.2 Functionality Testing
Table 4.2 Sample Template
4.3 Debugging techniques and resolved issues
6 RESULTS AND ANALYSIS
7 CONCLUSIONS
5.1 Conclusion
5.2 Recommendations
5.3 Key Learnings
5.3 Challenges and Solutions
APPENDICES
A REFERENCES
B PROJECT MANAGEMENT
Project Manager / Team Members
Project Timeline / Schedule
Resources and Tools
Project Goals & Deliverables

---

LIST OF FIGURES


---

LIST OF TABLES



---

